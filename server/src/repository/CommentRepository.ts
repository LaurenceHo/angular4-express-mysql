...
export default class CommentRepository implements BaseRepository<Comment> {
	...
	findOneById(id: number, callback: any): void {
		database.query('SELECT * FROM comments WHERE id = ?', [id]).then((result: any) => {
			callback({ comment: result[0] });
		});
	}

	createOne(item: Comment, callback: any): void {
		database.query('INSERT INTO comments SET ?', item).then((result: any) => callback({ comment_id: result.insertId }));
	}
